...
### Importing and Cleaning SHSAT Data from 2015-2016 to 2017-2018
...
first_three_df = map_dfc(file_names_df, load_shsat)

tester_df %>% 
  rbind(name_new_df = )

name_new = rbind(tester_df, new, new2)

```

#### Importing and Cleaning SHSAT Data from 2018-2019 to 2019-2021
...
in order to join to the theoretical shsat dataset

```{r}

ms_dir_import =
  read_csv("raw_data/2019_DOE_Middle_School_Directory.csv") %>% 
  select(district, schooldbn, printedschoolname, neighborhood, address, sharedbuilding, subway, bus)

ms_dir_df =
  ms_dir_import %>%
  rename(
    dbn = schooldbn,
    feeder_school = printedschoolname
  ) %>% 
...
shsat_zip_df =
  full_join(shsat_df, ms_zip_dbn, by = "dbn") %>% 
  drop_na() %>% 
  mutate(
