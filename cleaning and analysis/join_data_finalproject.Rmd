...
zcta_skool_covidtest <- full_join(zip_mod, school, by = "zip") %>% 
  rename(
     modzcta = MODZCTA
  ) %>% 
  select(-`...1`)
...
project_df_deathrate = 
  full_join(zcta_skool, covid_deathrate, by = "modzcta") %>% 
  janitor::clean_names()


write_csv(zcta_skool_covidtest, "project_df.csv")
write_csv(project_df_deathrate, "project_df_plus_deathrate.csv")
